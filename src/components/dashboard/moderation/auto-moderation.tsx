...
export function AutoModeration({ serverId }: { serverId: string }) {
	...
	const [settings, setSettings] = useState<ExtendedAutoModerationRule | null>(
		null,
	);
		...
		<div className="space-y-6">
			<div className="space-y-4">
				<ModerationRuleCard
					...
					icon="filter"
					enabled={
						settings.trigger_metadata?.keyword_filter?.length ? true : false
					}
					onToggle={(enabled) =>
						updateSettings({
							...settings,
						...
						updateSettings({
							...
							customSettings: {
								...
								spamSettings: {
									enabled,
									threshold:
										settings.customSettings?.spamSettings?.threshold || 5,
								},
							},
						})
					}
