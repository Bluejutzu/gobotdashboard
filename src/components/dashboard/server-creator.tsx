		...
		try {
			...
			const { data: newServer, error: createError } = await supabase
				...
				.insert({
					...
					icon_url: response.data.icon
						? `https://cdn.discordapp.com/icons/${discordId}/${response.data.icon}.png`
						: null,
	...
	if (error) {
		...
		return (
			<ServerError
				...
				message={
					error.message ||
					"Something went wrong while creating your server. Please try again later."
				}
				code={error.code}
			/>
		);
