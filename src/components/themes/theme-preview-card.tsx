...
export function ThemePreviewCard({ theme, onApply }: ThemePreviewCardProps) {
  return (
    <div className="border rounded-md p-4 bg-white/5 backdrop-blur-sm border-white/10">
      ...
      <p className="text-sm text-white/70">Border Radius: {theme.borderRadius}px</p>
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center">
        {color && <div className="w-4 h-4 rounded-full mr-2" style={{ backgroundColor: color }}></div>}
...
export function DifferenceItem({ label, value1, value2, color1, color2 }: DifferenceItemProps) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-sm font-medium text-white/70">{label}</span>
      <div className="flex items-center space-x-4">
        <div className="flex items-center">
          {color1 && <div className="w-4 h-4 rounded-full mr-2" style={{ backgroundColor: color1 }}></div>}
          <span className="text-sm">{value1}</span>
        </div>
        <span className="text-sm">→</span>
        <div className="flex items-center">
          {color2 && <div className="w-4 h-4 rounded-full mr-2" style={{ backgroundColor: color2 }}></div>}
