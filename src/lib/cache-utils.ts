 ...
 */
export async function cacheUserGuilds(
	userId: string,
...
export async function getCachedUserGuilds(
	userId: string,
): Promise<DiscordPartialGuild[] | null> {
	const key = `${CACHE_KEYS.USER_GUILDS}${userId}`;
	...
	try {
		...
		console.error(
			`Error processing cached guild data for user ${userId}:`,
			error,
		);
		...
		await redis.del(key);
		console.warn(
			`Invalidated corrupted cache (due to parse error) for key: ${key}`,
		);
		return null;
	}
}
...
export async function getCachedBotGuilds(): Promise<string[] | null> {
	...
	try {
		...
		if (typeof cachedData === "string") {
			...
			cachedData.every((item) => typeof item === "string")
		) {
		...
		if (typeof cachedData === "string") {
			console.error(
				"Problematic string data for bot guilds:",
				cachedData.substring(0, 100),
			);
		}
		await redis.del(key);
		console.warn(
 ...
 */
export async function cacheGuild(
	guildId: string,
	guildData: any,
): Promise<void> {
...
export async function getCachedGuild(guildId: string): Promise<any | null> {
	...
	try {
		...
		if (typeof cachedData === "string") {
			console.error(
				`Problematic string data for guild ${guildId}:`,
				cachedData.substring(0, 100),
			);
		}
		await redis.del(key);
 ...
 */
export async function cacheModerationCases(
	guildId: string,
 ...
 */
export async function getCachedModerationCases(
	guildId: string,
	...
	try {
		if (typeof cachedData === "string") {
			...
			console.error(
				`Unexpected cache format for guild ${guildId}:`,
				cachedData,
			);
			await redis.del(key);
			return null;
		...
		console.error(
			`Error parsing moderation cases for guild ${guildId}:`,
			error,
		);
		if (typeof cachedData === "string") {
 ...
 */
export async function invalidateModerationCasesCache(
	guildId: string,
): Promise<void> {
	const key = `${CACHE_KEYS.MODERATION_CASES}${guildId}`;
	await redis.del(key);
