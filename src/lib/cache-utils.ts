 ...
 *
 * Returns an array of {@link DiscordPartialGuild} if valid cached data exists, or null if the cache is missing or corrupted.
 *
 * @param userId - The Discord user ID whose guilds are being retrieved.
 * @returns The cached array of partial guilds, or null if not found or if the cache is invalid.
 */
export async function getCachedUserGuilds(userId: string): Promise<DiscordPartialGuild[] | null> {
    ...
    try {
        ...
        if (typeof cachedData === 'string') {
            ...
            console.error(`Error parsing cached guild data: cachedData for user ${userId} is neither a string nor an array. Type: ${typeof cachedData}`, cachedData);
...
export async function cacheBotGuilds(guildIds: string[]): Promise<void> {
    await redis.set(CACHE_KEYS.BOT_GUILDS, JSON.stringify(guildIds), { ex: CACHE_TTL.BOT_GUILDS })
    console.log(`Cached ${guildIds.length} bot guild IDs`)
}

/**
 * Retrieves the bot's cached guild IDs from Redis.
 *
 * Returns an array of guild ID strings if present and valid, or null if the cache is missing or corrupted.
 *
 * @returns An array of guild ID strings, or null if not found or if the cache is invalid.
 */
export async function getCachedBotGuilds(): Promise<string[] | null> {
    ...
    try {
        ...
        if (typeof cachedData === 'string') {
            ...
            console.error("Error parsing cached bot guild data: cachedData is not a string or an array of strings.", cachedData);
            await redis.del(key);
            console.warn(`Invalidated corrupted cache for key: ${key}`);
            return null;
        }
...
export async function getCachedGuild(guildId: string): Promise<any | null> {
    ...
    try {
        if (typeof cachedData === 'string') {
            ...
            console.error(`Error parsing cached data for guild ${guildId}: cachedData is not a string or object.`, cachedData);
            await redis.del(key);
            console.warn(`Invalidated corrupted cache for key: ${key}`);
            return null;
        }
