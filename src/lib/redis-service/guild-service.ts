...
/**
 * Retrieves the list of Discord guilds a user belongs to, filtered by management permissions and utilizing caching for efficiency.
 *
 * Attempts to return cached guilds unless a forced refresh is requested. If the Discord token is expired, automatically refreshes the token and retries up to three times. Only guilds where the user has administrative or management permissions are included.
 *
 * @param userId - The Discord user ID.
 * @param supabase_user_id - The corresponding Supabase user ID.
 * @param forceRefresh - If true, bypasses the cache and fetches fresh data.
 * @param src - Context string for logging purposes.
 * @returns An array of guilds where the user has management permissions.
 *
 * @throws {Error} If required user identifiers are missing, the Discord token cannot be retrieved, or the Discord API returns an error after retries.
 */
...
/**
 * Returns a list of servers the user can manage, including bot presence and guild details.
 *
 * Each server object includes the guild's ID, name, icon URL, permissions, whether the bot is present, member and presence counts, features, and stickers.
 *
 * @param userId - Discord user ID.
 * @param supabase_user_id - Supabase user ID associated with the Discord user.
 * @param forceRefresh - If true, bypasses cache and fetches fresh data.
 * @param src - Optional context string for logging.
 * @returns An array of servers the user can manage, with bot presence and metadata.
 *
 * @throws {Error} If fetching user or bot guilds fails.
 */
