...
export async function fetchUserGuilds(
    ...
    try {
        ...
        const response = await fetch("https://discord.com/api/v10/users/@me/guilds", {
            headers: {
                Authorization: `Bearer ${bearerToken.data.token}`
            }
        });
...
export async function getFormattedServerList(
    ...
    console.log(`[GUILD-SERVICE ${src}]: Getting formatted server list for ${userId}, ${supabase_user_id}`);
    try {
        // Fetch user guilds and bot guilds in parallel
        const [discordGuilds, botGuildIds] = await Promise.all([
            fetchUserGuilds(userId, supabase_user_id, forceRefresh),
