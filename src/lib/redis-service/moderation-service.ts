...
export async function deleteModerationCase(arg: NextRequest | string) {
    ...
    const { error } = await supabase.from("moderation_cases").delete().eq("id", id);
...
export const getAutoModerationSettings = async (serverId: string): Promise<AutoModerationRule | null> => {
    const raw = await getCache<string>(`${CACHE_KEYS.AUTO_MODERATION_SETTINGS}${serverId}`);
    return raw ? (JSON.parse(raw) as AutoModerationRule) : null;
};

export const updateAutoModerationSettings = async (serverId: string, settings: AutoModerationRule) => {
    await setCache(
        `${CACHE_KEYS.AUTO_MODERATION_SETTINGS}${serverId}`,
        JSON.stringify(settings), // let the cache layer handle serialisation
