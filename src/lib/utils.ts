...
export async function refreshDiscordToken(
	...
	success: boolean;
	error?: string;
	data?: { discord_access_token: any; discord_refresh_token: any };
}> {
	...
	try {
		...
		if (axios.isAxiosError(error)) {
			throw new Error(
				`Could not refresh Discord token: ${error.response?.data?.error || error.message}`,
			);
		}
	...
	if (refreshToken.status !== 200) {
		throw new Error("Could not refresh Discord token", {
			cause: refreshToken.statusText,
		});
	}
