...
export function validateThemeData(data: any): data is ThemeData {
    ...
    if (typeof data.primary !== "string" || !isValidHexColor(data.primary)) return false;
    if (typeof data.secondary !== "string" || !isValidHexColor(data.secondary)) return false;
    if (typeof data.accent !== "string" || !isValidHexColor(data.accent)) return false;

    // Check for required number properties
    if (typeof data.borderRadius !== "number" || data.borderRadius < 0) return false;
