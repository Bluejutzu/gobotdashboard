...
export async function saveTheme(theme: DBThemeData) {
    try {
        ...
        const { data, error } = await supabase.from("user_themes").insert([themeData]).select();
...
export async function updateTheme(id: number | string, theme: Partial<ThemeData>) {
    try {
        const updates = formatThemeForDb(theme as ThemeData);

        const { data, error } = await supabase.from("user_themes").update(updates).eq("id", id).select();
...
export function formatThemeForDb(theme: Partial<ThemeData>): any {
    ...
    if (theme.borderRadius !== undefined) dbTheme.border_radius = theme.borderRadius;
