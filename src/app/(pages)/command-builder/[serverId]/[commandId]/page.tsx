...
const CommandBuilder = () => {
			...
			prev.map((block) =>
				block.id === selectedBlock.id
					? {
							...
							label: updatedData.name || block.label,
						}
					: block,
			),
		);
		...
		setSelectedBlock((prev) =>
			prev
				? {
						...
						label: updatedData.name || prev.label,
					}
				: null,
		);
	};
		...
		setBlocks((prev) =>
			prev.map((block) => {
				if (block.id === connection.fromId) {
					// eslint-disable-next-line @typescript-eslint/no-unused-vars
					const { [connection.fromSocket]: _, ...restConnections } =
						block.connections;
					return {
						...block,
				...
				if (block.id === connection.toId) {
					// eslint-disable-next-line @typescript-eslint/no-unused-vars
					const { [connection.toSocket]: _, ...restConnections } =
						block.connections;
					return {
						...block,
						connections: restConnections,
	...
	const filteredBlocks = () => {
		...
		const categoryBlocks =
			blocksByCategory[activeTab as keyof typeof blocksByCategory] || [];
		...
		<div className="flex h-screen bg-[#1e2030] text-white overflow-hidden">
			...
			<div className="w-80 border-r border-[#2a2d3d] flex flex-col">
				...
				<div className="p-4 border-b border-[#2a2d3d]">
					<p className="text-sm text-gray-400 mb-2">
						...
						<span className="text-[#3b82f6]">Actions</span> and{" "}
						<span className="text-[#22c55e]">Conditions</span> to add them to
						your command. Connect the corresponding colors to create your
						command flow.
					</p>
				</div>
				...
				<div className="p-4 border-t border-[#2a2d3d]">
					<Button
						className="w-full bg-[#3b82f6] hover:bg-[#2563eb] hover-glow"
						onClick={() => {}}
					>
			...
			<div className="flex-1 flex flex-col">
				<div className="h-14 border-b border-[#2a2d3d] flex items-center justify-between px-4">
					<div className="flex items-center">
						<h2 className="text-lg font-medium mr-4">
							{isNewCommand ? "Creating new command" : "Editing command"}
						</h2>
						<Input
					...
					<div className="flex items-center gap-2">
						...
						<Button
							onClick={handleSave}
							disabled={isLoading}
							className="bg-[#3b82f6] hover:bg-[#2563eb] btn-pulse"
						>
				...
				<div
					...
					style={{
						backgroundImage: "radial-gradient(#2a2d3d 1px, transparent 1px)",
				...
				>
					...
					<svg className="absolute inset-0 pointer-events-none">
						{connections.map((connection) => {
							...
							if (
								fromBlock.blockType === "options" ||
								toBlock.blockType === "options"
							) {
								strokeColor = "#a855f7"; // Purple for options
							} else if (
								fromBlock.blockType === "conditions" ||
								toBlock.blockType === "conditions"
							) {
		...
		>
			...
			<div className="bg-[#252839] p-3 border border-[#2a2d3d] border-t-0">
				<div className="text-xs text-gray-300">
					{block.type === "command" && (
						<div>Command: /{block.data.name || "unnamed"}</div>
					)}
					{block.type === "send-message" && (
						<div className="truncate">
							{block.data.content || "No message content"}
						</div>
					)}
					{block.type === "embed-reply" && (
						<div className="truncate">
							{block.data.title || "No embed title"}
						</div>
					)}
					{block.type === "comparison" && (
						<div className="truncate">
							{block.data.leftValue || "[Value 1]"}{" "}
							{getComparisonOperator(block.data.comparisonType)}{" "}
							{block.data.rightValue || "[Value 2]"}
						</div>
					)}
					{block.type === "text-option" && (
						<div className="truncate">
							Option: {block.data.name || "unnamed"}
						</div>
					)}
					{block.type === "error-handler" && (
						<div className="truncate">Handles errors during execution</div>
					)}
				</div>
			</div>
			...
			<div
				className={cn(
					...
					isConnecting &&
						connectionStart?.id === block.id &&
						connectionStart.socket === "output"
						? "scale-125 animate-pulse-glow"
						: "hover:scale-110 hover:animate-pulse-glow",
				)}
				style={{ borderColor: blockColor }}
					...
					<div className="space-y-4 animate-fade-in">
						<div>
							...
							<p className="text-xs text-gray-400 mt-1">
								The name users will type to trigger this command.
							</p>
						</div>
						...
						<div>
							...
							/>
							<p className="text-xs text-gray-400 mt-1">
								Shown in the help menu and command list.
							</p>
						</div>
						...
						<div>
							...
							<div className="flex gap-2">
								<select
									...
									onChange={(e) =>
										onChange({
											cooldownType: e.target.value as "user" | "server",
										})
									}
								>
								...
								<Input
									...
									onChange={(e) =>
										onChange({ cooldown: Number.parseInt(e.target.value) || 0 })
									}
								/>
							</div>
						</div>
					...
					<div className="space-y-4 animate-fade-in">
						...
						<div>
							...
							<select
								...
								onChange={(e) =>
									onChange({
										comparisonType: e.target
											.value as BlockData["data"]["comparisonType"],
									})
								}
							>
				...
				return (
					<div className="space-y-4 animate-fade-in">
						<div>
							...
							/>
							<p className="text-xs text-gray-400 mt-1">
								This message will be shown when an error occurs.
							</p>
						</div>
					</div>
				);
	...
	return (
		<div className="space-y-6">
			...
			<div className="animate-fade-in animation-delay-300">
				...
				/>
				<p className="text-xs text-gray-400 mt-1">
					Add an optional label to this block. This will change how the block
			...
			{block.type !== "command" && (
				<div className="pt-4 border-t border-[#2a2d3d] animate-fade-in animation-delay-500">
					<Button
						variant="destructive"
						className="w-full bg-red-600 hover:bg-red-700 btn-pulse"
						onClick={onDelete}
					>
						Delete Block
					</Button>
