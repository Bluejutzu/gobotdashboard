...
export async function POST(req: Request) {
    ...
    if (!token || !refreshToken) {
        return new Response(JSON.stringify({ error: "Missing access or refresh token" }), { status: 400 });
    }
    ...
    try {
        ...
        return new Response(JSON.stringify({ error: "Failed to validate token with Discord" }), { status: 400 });
    }
