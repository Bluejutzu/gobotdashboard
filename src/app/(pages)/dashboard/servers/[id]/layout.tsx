...
/****
 * Renders the main layout for a server dashboard page, managing user authentication, server selection, and access-controlled navigation.
 *
 * Displays a responsive sidebar and mobile menu for navigating between server features, handles user session state, and fetches both current server details and the user's list of manageable servers. Shows beta alerts for experimental features and restricts access to developer-only sections as appropriate.
 *
 * @param children - The content to render within the main dashboard area.
 * @param params - Route parameters containing the current server ID.
 *
 * @remark Redirects to the login page if the user is not authenticated or user data is missing.
 */
export default function ServerLayout({ children, params }: ServerLayoutProps) {
	...
	const [currentBetaFeature, setCurrentBetaFeature] = useState<string | null>(
		null,
	);
	const [serverSelectorOpen, setServerSelectorOpen] = useState(false);

	const fetchUserServers = React.useCallback(
		async (userId: string, supabase_user_id: string) => {
			if (!userId || !supabase_user_id) {
				console.warn("Missing required parameters for fetchUserServers", {
					userId,
					supabase_user_id,
				});
				return;
	...
	const isSiteDeveloper =
		userData?.discord_id && SITE_DEVELOPERS.includes(userData.discord_id);

	const canAccessNavItem = (item: NavItem) => {
		if (!userData) {
			// If user data is not yet loaded, assume restricted access for experimental features
	...
	const navItems: NavItem[] = React.useMemo(
		() => [
			...
			{
				href: `/dashboard/servers/${id}/commands`,
				label: "Commands",
				icon: MessageSquare,
				experimental: "no_access",
			},
			{
				href: `/dashboard/servers/${id}/messages`,
				label: "Messages",
				icon: MessageSquare,
				experimental: "no_access",
			},
			...
			{
				href: `/dashboard/servers/${id}/moderation`,
				label: "Moderation",
				icon: Shield,
				experimental: "no_access",
			},
			...
			{
				href: `/dashboard/servers/${id}/join-roles`,
				label: "Join Roles",
				icon: UserPlus,
				experimental: "no_access",
			},
			...
			{
				href: `/dashboard/servers/${id}/connections`,
				label: "Role Connections",
				icon: Link2,
				experimental: "no_access",
			},
			{
				href: `/dashboard/servers/${id}/logging`,
				label: "Logging",
				icon: FileText,
				experimental: "no_access",
			},
			...
			{
				href: `/dashboard/servers/${id}/ai-tools`,
				label: "AI Tools",
				icon: Sparkles,
				experimental: "no_access",
			},
							...
							<div className="px-4 mb-6">
								<DropdownMenu>
									<DropdownMenuTrigger asChild>
										<Button
											variant="ghost"
											className="w-full justify-start text-left font-semibold text-lg gap-2 px-2"
										>
									...
									<DropdownMenuContent className="w-56" align="start">
										...
										{serversLoading ? (
											...
											<ScrollArea className="h-[300px]">
												{userServers.map(
													(
														s, // Renamed to 's' to avoid conflict if 'server' is in wider scope
													) => (
														<DropdownMenuItem
															key={s.id}
										...
										<DropdownMenuItem asChild>
											<Link href="/dashboard/guilds" className="cursor-pointer">
												{" "}
												{/* This link might be where users add/manage servers */}
							...
							<div className="flex-1 px-2 space-y-1 overflow-y-auto">
								{accessibleNavItems.map((item, i) =>
									item.type === "header" ? (
										<div
											key={i}
											className="px-3 py-2 text-xs font-semibold text-slate-500 mt-4"
										>
											{item.label}
										</div>
									) : (
												...
												<TooltipContent side="right">
													{item.experimental === "beta" && (
														<p className="flex items-center gap-1">
															...
															<span>
																This feature is in beta and may not work as
																expected
															</span>
														</p>
													)}
							...
							item.type === "header" ? (
								...
								<TooltipProvider key={i}>
									<Tooltip>
										...
										<TooltipContent side="right">
											{item.label}{" "}
											{/* Simplified tooltip content, or use experimental checks as before */}
											{item.experimental === "beta" && (
												<p className="flex items-center gap-1">
													<AlertTriangle className="h-3 w-3 text-yellow-400" />
			...
			<main className="flex-1 p-4 md:p-8 pt-20 md:pt-8">
				...
				{showBetaAlert && currentBetaFeature && (
					<Alert className="mb-4 border-yellow-500/50 text-yellow-200 bg-yellow-900/30 [&>svg]:text-yellow-400">
						...
						<AlertDescription>
							The &quot;{currentBetaFeature}&quot; feature is currently in beta.
